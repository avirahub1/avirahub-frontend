...
async function getAboutData(): Promise<any> {
    try {
        ...
        const data = await CMS.findOne({ section: 'about' }).lean();
...
async function getTeamHeaderData(): Promise<any> {
    try {
        await connectToDatabase();
        const data = await CMS.findOne({ section: 'about_team' }).lean();
        return serializeData(data) || {};
    } catch (error) {
        return {};
    }
...
// Check for active team members from API
async function getTeamMembers(): Promise<ITeamMember[]> {
    try {
        await connectToDatabase();
        // Assuming GET /api/team returns active members sorted by order
        const team = await Team.find({ active: true }).sort({ order: 1 }).lean();
...
export default async function AboutPage() {
    ...
    const teamHeaderData = await getTeamHeaderData();
